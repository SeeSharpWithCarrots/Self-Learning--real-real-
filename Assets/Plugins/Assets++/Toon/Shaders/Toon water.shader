...
Shader "JP Toon" {
    Properties {
        ...
        _Opacity ("Opacity", Range(0, 1)) = 0.5343278
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 Movement = ((float2(_Uspeed,_Vspeed)*node_6163.g)+i.uv0);
                float2 node_2977 = Movement;
                float3 _ToonBump_var = UnpackNormal(tex2D(_ToonBump,TRANSFORM_TEX(node_2977, _ToonBump)));
                float3 NormalTex = _ToonBump_var.rgb;
                float3 normalLocal = NormalTex;
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float3 viewReflectDirection = reflect( -viewDirection, normalDirection );
                float2 node_4110 = Movement;
                float4 _refractionmap_var = tex2D(_refractionmap,TRANSFORM_TEX(node_4110, _refractionmap));
...
////// Lighting:
                float attenuation = 1;
////// Emissive:
                float2 node_8442 = Movement;
                half4 _EmissionToon_var = tex2D(_EmissionToon,TRANSFORM_TEX(node_8442, _EmissionToon));
                ...
                float2 node_6148 = Movement;
                half4 _DiffuseTextue_var = tex2D(_DiffuseTextue,TRANSFORM_TEX(node_6148, _DiffuseTextue));
                ...
                float2 node_7141 = Movement;
                float4 _SpecularTex_var = tex2D(_SpecularTex,TRANSFORM_TEX(node_7141, _SpecularTex));
