...
Shader "Shader Forge/Examples/Custom Lighting" {
    Properties {
        ...
        _Gloss ("Gloss", Range(0, 1)) = 0.5609585
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
...
////// Lighting:
                ...
                float3 finalColor = (attenuation*_LightColor0.rgb*((_Diffuse_var.rgb*_Color.rgb*floor(max(0,dot(lightDirection,normalDirection)) * _Bands) / (_Bands - 1))+UNITY_LIGHTMODEL_AMBIENT.rgb+floor(pow(max(0,dot(normalDirection,halfDirection)),exp2(((_Gloss*2.0)+1.0))) * _Bands) / (_Bands - 1)));
